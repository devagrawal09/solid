...
- Round 3 (async status, stores, actions) and runtime-only speculation: store scalar replacement is the largest per-site win measured (mount −76%, but 2 of 15 stores qualify), status pass-through composes with fusion (async refetch −44%), and synchronous actions compiled to a plain batch save 15%. Runtime-only attempts at the same shortcuts break 9–528 tests or regress, except two: status-free recompute promoted after a clean run (R1b) closes the select gap (−22%, DOM −33%) but costs 1–2% at mount, and a selection projection beats fusion on select (−85%). See the ledger in [heuristic-oracles.md](./heuristic-oracles.md).
